...
describe("guest silhouette art", () => {
  ...
  it("五套肖像变体文件均存在", () => {
    expect(guestPortraitVariantCount()).toBe(5);
    expect(GUEST_PORTRAIT_VARIANTS.length).toBe(5);
  ...
  it("同名确定性选图，异名可分散", () => {
    ...
    expect(guestPortraitDataAttr("阿澄")).toMatch(/^v[0-4]$/);
